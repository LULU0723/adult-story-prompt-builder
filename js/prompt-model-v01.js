...
function buildStages(generation, byId) {
  ...
  for (const step of generation?.results ?? []) {
    ...
    grouped.get(step.stage).push({
      kind: step.kind,
      forced: Boolean(step.forcedItemId),
      text: renderTemplate(step, byId)
    });
  }
  return [1, 2, 3]
    .filter(stage => grouped.get(stage).length)
    .map(stage => ({ stage, label: STAGE_LABELS[stage], beats: grouped.get(stage) }));
}
...
export function buildPromptModel({ generation, characters, sceneConfig = {}, storyConfig = {} } = {}) {
  ...
  return {
    characters: normalizedCharacters.map(describeCharacter),
    scene: describeScene(sceneConfig),
    controls: [
      `- 建議篇幅：${readConfig(LENGTH_TEXT, config.length, 'short')}`,
      `- 開場：${readConfig(OPENING_TEXT, config.opening, 'direct')}`,
      `- 節奏：${readConfig(PACE_TEXT, config.pace, 'quick_escalation')}`,
      `- 寫作風格：${readConfig(STYLE_TEXT, config.writingStyle, 'character_driven')}`,
...
function pushDynamicSections(lines, model) {
  lines.push('【角色設定】');
  ...
  lines.push('');
  lines.push('【敘事控制】');
  lines.push(...model.controls);
  lines.push('');
  lines.push('【主軸】');
  if (model.anchor) {
    lines.push(`本篇核心：${model.anchor.label}（第 ${model.anchor.stage} 階段）`);
  } else {
...
const PER_RUN_CONSISTENCY = Object.freeze([
  '- 保持角色身體設定、道具所有權、角色方向與行動限制前後一致。',
  '- 不要替角色新增未提供的身體條件或道具，也不要把其中一人的身體條件錯算到另一人。',
  '- 性別代稱與第二人稱用語應依角色的「性別設定」保持一致，不得由身體設定、攜帶道具或外在呈現反推性別；非二元或不指定時也不要自行推定。',
  '- 若節點包含角色方向，執行時要維持指定的主動方與接受方；雙方互動節點則不要硬改寫成單方面支配。'
]);

export function renderStandalonePrompt(model) {
  const lines = [];
  lines.push('【任務】');
  ...
  lines.push('');
  lines.push('【主軸優先級】');
  if (model.anchor) lines.push('主軸是本篇最重要的互動核心；前後內容應鋪墊、強化或回應它，具體行動只在對應階段執行。');
  else lines.push('沒有可用主軸時，不要自行新增未指定的核心玩法。');
  lines.push('');
  lines.push('【輸出方式】');
